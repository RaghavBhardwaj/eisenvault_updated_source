...
======================================================================
ReadMe do LibreOffice 4.2
======================================================================


Para as últimas atualizações a este ficheiro, consulte http://www.libreoffice.org/welcome/readme.html
...
A comunidade LibreOffice é responsável pelo seu desenvolvimento e incentiva todos os utilizadores a integrar a comunidade. Se você é um novo utilizador, pode visitar a página do LibreOffice, onde vai encontrar informações sobre o projeto e as comunidades do LibreOffice. Consulte a página http://www.libreoffice.org/.
...
Tenha em atenção que o processo de instalação requer direitos de administrador.
...
Certifique-se que tem memória suficiente no diretório temporário do seu sistema e que tem permissão para escrever, ler e executar. Feche todos os programas antes de iniciar a instalação.
...
Ao extrair o arquivo transferido, irá verificar que o conteúdo foi descomprimido para um subdiretório. Abra uma janela do gestor de ficheiros e altere o diretório para aquele que começa com "LibreOffice_", seguido pelo número da versão e algumas informações da plataforma.
...
Ao extrair o arquivo transferido, irá verificar que o conteúdo foi descomprimido para um subdiretório. Abra uma janela do gestor de ficheiros e altere o diretório para aquele que começa com "LibreOffice_", seguido pelo número da versão e algumas informações sobre a plataforma.
...
Deve ser fácil instalar o LibreOffice nos sistemas Linux que não estão abrangidos pelas instruções de instalação. As principais diferenças devem ocorrer na integração ao ambiente de trabalho.

O diretório RPMS (ou DEBS, respetivamente) também contém um pacote com o nome libreoffice4.1-freedesktop-menus-4.1.0-3.noarch.rpm (ou libreoffice4.1-debian-menus_4.1.0-3_all.deb, respetivamente). Este é o pacote para distribuições Linux que tenham suporte às especificações/recomendações Freedesktop.org (http://en.wikipedia.org/wiki/Freedesktop.org), servindo também para as distribuições Linux não abrangidas pelas instruções apresentadas.

Instalar um pacote de idioma
----------------------------------------------------------------------

Transfira o pacote do idioma pretendido. Estes pacotes estão disponíveis no mesmo local do pacote de instalação. Agora, no gestor de ficheiros Nautilus, extraia o arquivo transferido para um diretório (ambiente de trabalho, por exemplo). Certifique-se que encerrou todas as aplicações do LibreOffice (não se esqueça do Início rápido, se ativo).

Altere o diretório para aquele em que você extraiu o pacote de idioma transferido.
...
A partir do gestor de ficheiros Nautilus, clique com o botão direito do rato e escolha "Abrir no terminal". Na janela de terminal que abriu, execute o comando para instalar o pacote (com todos os comandos em baixo, pode ser-lhe solicitada a palavra-passe de "root"):
...
Após ajustar todas as definições pretendidas, clique Aceitar. A janela vai ser fechada e você vai receber uma mensagem a alertar que a interface do utilizador foi atualizada e que será aplicada na próxima vez que iniciar o LibreOffice (não se esqueça de fechar o QuickStarter, se ativo).
...
Apenas as teclas de atalho (combinação de teclas) que não são utilizadas pelo sistema operativo, podem ser utilizadas no LibreOffice. Se uma combinação de teclas do LibreOffice não funcionar como descrito na ajuda do LibreOffice, verifique se o atalho está a ser utilizado pelo sistema operativo. Para resoilver os conflitos, pode alterar as teclas atribuídas pelo seu sistema operativo. Em alternativa, pode alterar quase todas as atribuição de teclas do LibreOffice. Para mais informações sobre este tópico, consulte a ajuda do LibreOffice ou a documentação do seu sistema operativo.

----------------------------------------------------------------------
Problemas ao enviar documentos como correio eletrónico no LibreOffice
----------------------------------------------------------------------
...
A página de suporte http://www.libreoffice.org/support/ oferece diversas formas de ajuda para o LibreOffice. A sua dúvida pode até já ter sido respondida - verifique o fórum da comunidade em http://www.documentfoundation.org/nabble/ ou procure nos arquivos da lista de correio "users@libreoffice.org" em http://www.libreoffice.org/lists/users/. Em alternativa, pode enviar as suas dúvidas para users@libreoffice.org. Se pretender subscrever à lista, envie uma mensagem sem texto para: users+subscribe@libreoffice.org.
...
Esperamos que o novo LibreOffice 4.2 corresponda às suas expectativas e aguardamos a sua integração na equipa.
